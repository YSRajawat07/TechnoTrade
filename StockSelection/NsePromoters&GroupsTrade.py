...
import numpy as np
from NseDataFetch import funstockprice
from utility import *
...
filename = config.get('filedetails','NseSwingTrade.filename')
excel_file = path+filename

wb = xw.Book(excel_file)
sheet_oi_single = wb.sheets("promoters")
...
sdt =datetime.today()-timedelta(days=90)
...
enddate = str(dt.day)+"-"+str(dt.month)+"-"+str(dt.year)
...
def positions():
  ...
  #  print(dfsymbol)
    # df_buy =df[df['acqMode'] =='Market Purchase']
    # df_sell =df[df['acqMode'] =='Market Sale']
...
#    dfSell.sort_values(by=['date'], ascending=False,inplace=True)
 ...
 #   dfCombi = np.where((dfCombi['percSharesPledged'].apply(lambda x: float(x)) <=0.5))
   # print(dfCombi.head(1))
    dfCombi=pd.merge(dfCombi, stockprice, how='inner', on='symbol')
    print('Printing data frame')

    print(dfCombi)

    dfCombi['up/down%'] =  round((dfCombi.avgBuyPrice.astype(int) / dfCombi.Price.astype(int))*100, 2)
    wb.sheets['swingdetails'].range("A1").options(index=False, header=True).value = df
    wb.sheets['promoters'].range("A1").options(index=True, header=True).value = dt
    wb.sheets['promoters'].range("A3").options(index=True, header=True).value = dfCombi[['symbol','date','avgBuyPrice','Price','noOfShareSale','percPromoterHolding','percSharesPledged','up/down%']]
