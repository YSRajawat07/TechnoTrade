...
excel_file = "/Users/ajaysinghrajawat/Documents/pythonprojects/Files/NSE_OptionChainStocks.xlsx"
...
oi_filename = os.path.join("/Users/ajaysinghrajawat/Documents/pythonprojects/Files/Intraday/Daily",
                           "OINS_" + stksymbol +"_records_{0}".format(datetime.now().strftime("%d%m%y")))
mp_filename = os.path.join("/Users/ajaysinghrajawat/Documents/pythonprojects/Files/Intraday/Daily",
                           "MPNSE_" + stksymbol +"_records_{0}".format(datetime.now().strftime("%d%m%y")))
...
def getoptionchain(df, mp_df, df_cdata, df_pdata):
    ...
    while tries <= max_retries:
        try:
            ...
            url_FO = "https://www1.nseindia.com/live_market/dynaContent/live_watch/stock_watch/foSecStockWatch.json"

            rfo = requests.get(url_FO, headers=headersfo).json()

            ce_values = rfo['data']
            ce_data = pd.DataFrame(ce_values)


            url = ( "https://www1.nseindia.com/live_market/dynaContent/live_watch/option_chain/optionKeys.jsp?symbolCode=-"
                        "&symbol="+stksymbol+"&instrument=OPTSTK&date=" + expirystock + "&segmentLink=17&symbolCount=2&segmentLink=17")
            ...
            if instr == "Index":
                ...
                df_spot= ce_data[ce_data['symbol']==stksymbol]

                niftyspot= df_spot.iloc[0][4]
            ...
            callDecay = (df[df['callsOTM'] == True]).nlargest(3, 'OI_Call', keep='last')['LTP_Call'].astype(float)
            callOI = (df[df['callsOTM'] == True]).nlargest(3, 'OI_Call', keep='last')['OI_Call']

            callvol = (df[df['callsOTM'] == True]).nlargest(3, 'OI_Call', keep='last')['Volume_Call']
            callCHOI = (df[df['callsOTM'] == True]).nlargest(3, 'OI_Call', keep='last')['Chng_in_OI_Call']
            IVdatacall = (df[df['callsOTM'] == True]).nlargest(3, 'OI_Call', keep='last')['IV_Call'].astype(float)
            callstrikepice = (df[df['callsOTM'] == True]).nlargest(3, 'OI_Call', keep='last')['Strike_Price']

            pcr = (df[df['putsOTM'] == True].nlargest(3, 'OI_Put', keep='last')['OI_Put'].sum()) / \
                  (df[df['callsOTM'] == True].nlargest(3, 'OI_Call', keep='last')['OI_Call'].sum())

            PutDecay = (df[df['putsOTM'] == True]).nlargest(3, 'OI_Put', keep='last')['LTP_Put'].astype(float)
            putOI = (df[df['putsOTM'] == True]).nlargest(3, 'OI_Put', keep='last')['OI_Put']

            putvol = (df[df['putsOTM'] == True]).nlargest(3, 'OI_Put', keep='last')['Volume_Put']
            putCHOI = (df[df['putsOTM'] == True]).nlargest(3, 'OI_Put', keep='last')['Chng_in_OI_Put']
            IVdataput = (df[df['putsOTM'] == True]).nlargest(3, 'OI_Put', keep='last')['IV_Put'].astype(float)
            putstrikeprice = (df[df['putsOTM'] == True]).nlargest(3, 'OI_Put', keep='last')['Strike_Price']


            # Max pain calculation starts here

            df['sumOI'] = df['OI_Call'] + df['OI_Put']
            ...
            wb.sheets['Dashboard'].range("C2").options(index=False, header=False).value=df_mpdata['Buy'].iloc[1]
            wb.sheets['Dashboard'].range("D2").options(index=False, header=False).value=df_mpdata['Sell'].iloc[1]
            ...
            wb.sheets['Dashboard'].range("F2").options(index=False, header=False).value=expirystock
...
def main():
    ...
    while time(9, 15) <= datetime.now().time() <= time(15, 32):
        ...
        check = True if round(int(str(timenow.minute)) / timeframe) in list(np.arange(0.00, 6.00)) else False
